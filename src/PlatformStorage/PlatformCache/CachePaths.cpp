...
{
  const spio::platform::OperatingSystemAdapter &os = spio::platform::DefaultOperatingSystemAdapter();
  if (const std::optional<std::string> explicit_home = os.GetEnv("SPIO_HOME");
      explicit_home.has_value() && !explicit_home->empty())
  {
    return CanonicalAbsolutePath(*explicit_home);
  }
  if (const std::optional<std::string> home = os.GetEnv("HOME"); home.has_value() && !home->empty())
  {
