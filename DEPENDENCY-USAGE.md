...
- The runtime `Containerfile` builds `styio-platformd`, `pafio`, and `styio` from source using OCI-compatible Podman or Buildah tooling and fails if the expected binaries are not produced. The source repositories and refs are explicit build arguments and default to the public `eBioRing` `ai-dev` branches.
